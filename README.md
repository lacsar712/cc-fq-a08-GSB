...
| 流水线 | `ParseActor` → `QualityHistActor` → `NContentActor` → `GcContentActor` → `ReportActor`（asyncio.Queue） |
...
- `demo-good-r1`：合格样例（可算出 `mean_quality` / `n_rate` / `gc_rate`）
...
3. 作业详情页看到五个 Actor 阶段均为成功（含报告前的 `GcContentActor`），指标卡出现 `reads` / `mean_quality` / `n_rate` / `gc_rate` / `gc_count`。
...
覆盖：畸形 FASTQ 在 `ParseActor` 失败（其后阶段含 `GcContentActor` 全部 skipped）；正常样例产出 `mean_quality` 与 `gc_rate`。
